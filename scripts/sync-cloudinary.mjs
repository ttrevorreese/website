...
`
  ...
  // ─── Hero photos ───────────────────────────────────────────────────────────
  // Preserves existing order in hero-photos.ts — only appends photos that
  // aren't already listed, so manual curation is never overwritten.
  process.stdout.write(`\nFetching hero photos from "${HERO_FOLDER}"... `)
  try {
    const heroResources = await fetchFolderPhotos(HERO_FOLDER)

    // Read the current file to find which URLs are already present
    const heroPath = resolve(ROOT, "content", "hero-photos.ts")
    let existingContent = ""
    try { existingContent = (await import("fs")).readFileSync(heroPath, "utf8") } catch {}

    // Extract existing src URLs so we don't duplicate them
    const existingUrls = new Set(
      [...existingContent.matchAll(/src:\s*"([^"]+)"/g)].map(m => m[1])
    )

    const newResources = heroResources.filter(r => {
      const url = cdnUrl(`${r.public_id}.${r.format}`, "f_auto,q_auto,w_800")
      return !existingUrls.has(url)
    })

    if (newResources.length === 0) {
      console.log(`no new photos (${heroResources.length} already listed)`)
    } else {
      // Append new entries before the closing ]
      const newEntries = newResources
        .map(r => {
          const url = cdnUrl(`${r.public_id}.${r.format}`, "f_auto,q_auto,w_800")
          return `  {\n    src: "${url}",\n    width: ${r.width},\n    height: ${r.height},\n  },`
        })
      ...
      const updatedContent = existingContent.trimEnd().replace(/\][\s]*$/, `${newEntries}\n]\n`)
      const { writeFileSync } = await import("fs")
      writeFileSync(heroPath, updatedContent, "utf8")
      console.log(`+${newResources.length} new photos appended → content/hero-photos.ts`)
    }
  } catch (err) {
    console.log(`SKIPPED (${err.message})`)
